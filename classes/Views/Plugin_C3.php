...
class Plugin_C3 {
		...
		if ( $args && ! empty( $args ) ) {
			foreach ( $args as $key => $value ) {
				if ( $value instanceof Environment ) {
					$this->env = $value;
				} else if ( $value instanceof Plugins ) {
                    $this->plugins = $value;
                }
			}
		}
		if ( ! isset( $this->env ) ) $this->env = new Environment();
        if ( ! isset( $this->plugins ) ) $this->plugins = new Plugins();
    ...
    public function render_html() {
        ...
        if ( isset( $additional_html ) ) echo $additional_html;
        echo '</div>';
        echo '</div>';
    }
}