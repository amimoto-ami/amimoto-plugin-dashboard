		...
		if ( $args && ! empty( $args ) ) {
			foreach ( $args as $key => $value ) {
				if ( $value instanceof Plugins ) {
					$this->plugin = $value;
				} else if ( $value instanceof Admin_Notice ) {
                    $this->notice = $value;
                }
			}
		}

        if ( ! $this->plugin || ! isset( $this->plugin ) ) $this->plugin = new Plugins();
        if ( ! $this->notice || ! isset( $this->notice ) ) $this->notice = new Admin_Notice();
        add_action(
			'admin_init',
			array(
				$this,
